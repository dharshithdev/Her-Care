...
import PregnancyView from "./PregencyView"; // Ensure the filename matches your actual file exactly
...
const TrackPage = () => {
    ...
    const { currentPhase, nextPeriodIn, prediction, fullCycleData, pregnant } = trackingData;
    // DATA HELPERS (Kept outside return so both views can use them if needed)
    const getFlowData = () => {
        const data = [];
                    ...
                    <>
                        ...
                        <section className="bg-white p-6 lg:p-10 rounded-[2.5rem] shadow-sm border border-slate-100 relative overflow-hidden">
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mt-4 relative z-10">
                                <button onClick={() => handlePeriodLog("regular")} className="py-5 px-6 rounded-2xl bg-rose-500 text-white font-black hover:bg-rose-600 transition-all text-xs tracking-wider active:scale-95">I GOT MY PERIODS TODAY</button>
                                <button onClick={() => handlePeriodLog("heavy")} className={`py-5 px-6 rounded-2xl font-bold border-2 text-xs tracking-wider transition-all ${selectedFlow === "heavy" ? "bg-rose-100 border-rose-500 text-rose-600" : "bg-white hover:bg-slate-100 text-slate-500 border-slate-50"}`}>HEAVY FLOW</button>
                                <button onClick={() => handlePeriodLog("regular")} className={`py-5 px-6 rounded-2xl font-bold border-2 text-xs tracking-wider transition-all ${selectedFlow === "regular" ? "bg-rose-100 border-rose-500 text-rose-600" : "bg-white hover:bg-slate-100 text-slate-500 border-slate-50"}`}>REGULAR PERIOD</button>
                            </div>
                                    ...
                                    <div key={index} className="flex flex-col items-center group relative">
                                        <div className="relative w-full h-[150px] lg:h-[200px]">
                                            <ResponsiveContainer width="100%" height="100%" key={`pie-${index}-${pieKey}`}>
                                                <PieChart>
                                                    <Pie data={[phase, { name: "Rem", value: phase.total - phase.value }]} dataKey="value" innerRadius="70%" outerRadius="90%" startAngle={90} endAngle={-270} stroke="none" animationDuration={1000}>
