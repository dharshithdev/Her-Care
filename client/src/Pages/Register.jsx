...
const Register = () => {
    ...
    const validateForm = () => {
        ...
        if (!formData.lastDate) {
            newErrors.lastDate = "Last date of periods required";
        }
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (error.response) {
                ...
                setFormMessage({ type: 'error', text: 'No response from server. Please check your network.' });
            } else {
        ...
        <div className="min-h-screen bg-pink-100 flex flex-col items-center justify-center py-8 px-4">
            ...
            <div className="max-w-6xl w-full flex items-stretch mx-auto">
                ...
                <div className="w-full md:w-1/2 bg-white rounded-xl shadow-lg p-6 md:p-10 overflow-hidden">
                    {formMessage.text && (
                        <div
                            className={`mb-4 p-3 rounded-md text-sm w-full max-w-md text-center ${formMessage.type === 'success' ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'
                                }`}
                            role="alert"
                            aria-live="polite"
                        >
                    ...
                    <form onSubmit={handleSubmit} className="space-y-4" noValidate>
                        ...
                        <div>
                            <label htmlFor="lastDate" className="block text-sm font-medium text-gray-700 mb-1">Last date of periods</label>
                            <input
                                type="date"
                                id="lastDate"
                                name="lastDate"
                                ...
                                className={`w-full px-4 py-2 border rounded-lg focus:outline-none focus:ring-2 transition-colors ${errors.lastDate ? 'border-red-500 focus:ring-red-300' : 'border-gray-300 focus:ring-pink-300'}`}
                                aria-describedby={errors.lastDate ? "date-error" : undefined}
                                aria-invalid={!!errors.lastDate}
                            />
                            {errors.lastDate && <p id="date-error" className="mt-1 text-xs text-red-600" role="alert">{errors.lastDate}</p>}
                        </div>
                        ...
                        <button
                            type="submit"
                            className="w-full bg-pink-500 text-white py-2 rounded-lg hover:bg-pink-600 transition disabled:opacity-50 flex items-center justify-center"
                            disabled={isLoading}
                        >
